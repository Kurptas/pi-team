...
export function workerRadioPrompt(systemPrompt: string, context?: WorkerPromptContext): string {
    return [
        ...
        "- Final output must still follow the requested structured finding fields.",
        "- When searching code or files, prefer the `grep` and `find` tools (available if inherited) over shell commands like `rg` or `ack`.",
    ].join("\n");
}
